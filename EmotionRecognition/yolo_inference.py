  ...
  for idx, line in enumerate(lines):
    ...
    try:
      ...
      for pred_bbox in bbox_yolo:
        ...
        #inference emotion categories
        for i, emotion in enumerate(pred_cat):
          cv2.putText(image_context, emotion, (pred_bbox[0], pred_bbox[1] + (i+1)*15), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 0), 1)
  ...
  while True:    
    ...
    try: 
      ...
      for pred_idx, pred_bbox in enumerate(bbox_yolo):
        ...
        ## warn Catagory에 해당하는 감정이 4개 이상이면 WARNING 문구 띄우기
        if(warn_num>3):
